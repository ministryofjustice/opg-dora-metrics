...
def workflow_runs_by_name_fuzzy(workflow_pattern:str, r:Repository, date_range:str = None, branch:str = "main", status:str = None) -> list[WorkflowRun]:
    ...
    logging.info(f"[{r.full_name}] looking for workflow runs for [{workflow_pattern}] during [{date_range}] on [{branch}]")
    workflows:list[WorkflowRun] = workflow_runs(r, date_range, branch, status)
    filtered:list[WorkflowRun] = []

    logging.info(f"[{r.full_name}] filtering workflow runs on name matching [{workflow_pattern}]")
    for workflow in workflows:
        if re.search(workflow_pattern, workflow.name.lower() ):
            logging.debug(f"[{r.full_name}] [workflow:{workflow.name}] matches [{workflow_pattern}]")
            filtered.append(workflow)
        else:
            logging.debug(f"[{r.full_name}] [workflow:{workflow.name}] does not match [{workflow_pattern}]")

    return filtered

def workflow_runs_by_name(workflow_name:str, r:Repository, date_range:str = None, branch:str = "main", status:str = None) -> list[WorkflowRun]:
    ...
    logging.info(f"[{r.full_name}] looking for workflow runs for [{workflow_name}] during [{date_range}] on [{branch}]")
    workflows:list[WorkflowRun] = workflow_runs(r, date_range, branch, status)
    filtered:list[WorkflowRun] = []

    logging.info(f"[{r.full_name}] filtering workflow runs on name matching [{workflow_name}]")
    for workflow in workflows:
        if workflow.name.lower() == workflow_name.lower():
            logging.debug(f"[{r.full_name}] [workflow:{workflow.name}] matches [{workflow_name}]")
            filtered.append(workflow)
        else:
            logging.debug(f"[{r.full_name}] [workflow:{workflow.name}] does not match [{workflow_name}]")
...
def workflow_runs(r:Repository, date_range:str, branch:str = "main" , status:str = None) -> list[WorkflowRun]:
    ...
    logging.info(f"[{r.full_name}] looking for workflow runs during [{date_range}] on [{branch}]")
    ...
    for workflow in workflows:
        logging.debug(f"[{r.full_name}] [workflow:{workflow.name}] [date:{workflow.created_at}]")
...
def workflow_total_durations(r:Repository, workflow_id:int) -> dict:
    ...
    for job in jobs:
        ...
        logging.debug(f"[{r.full_name}] [workflow:{workflow.name}] [job:{job.name}] duration: [{job_durations.get('seconds')}]")

    logging.info(f"[{r.full_name}] [workflow:{workflow.name}] >> total duration: [{total_durations.get('seconds')}]")
